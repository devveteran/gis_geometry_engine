...
double HereMapsLonToX(double longitude)
{
  return ((longitude + 180.0) / 360.0);
}

double HereMapsLatToY(double latitude)
{
  double latRad = latitude * M_PI / 180.0;
  return (1.0 - (log(tan(latRad) + 1 / cos(latRad)) / M_PI)) / 2.0;
}

std::pair<double, double> HereMapsGetXTileAndYTile(double latitude, double longitude, int zoomLevel)
{
  double n = pow(2, zoomLevel);

  double x = HereMapsLonToX(longitude);
  double y = HereMapsLatToY(latitude);

  double xTile = n * x;
  double yTile = n * y;

  std::pair<double, double> result(xTile, yTile);
